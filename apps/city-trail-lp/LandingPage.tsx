...
- 出力フォーマット:
{
  "title": "映画予告編のように引き込むタイトル（1行）",
  "description": "500-800字の長文説明（3-5段落、最後は問いかけ、段落は\\n\\n区切り）",
  "area": "主なエリア名",
  "difficulty": "初級|中級|上級",
  "target": "想定ターゲットを具体的に",
  ...
  "routeSpots": [
    {
      "name": "スポット名（Google Mapsで単独のスポットとして表示される正式名称）",
...
1. スポット間距離の制約（最重要）:
   ...
   - 各スポットはGoogle Mapsで単独のスポットとして表示される固有名を使うこと（施設名/店名/駅名/公園名など）
   - 「渋谷周辺」「駅前一帯」のような曖昧なエリア名は禁止
